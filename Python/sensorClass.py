...
def getWattage(value, voltage):
	...
	# changed 'value = (((float(value)/32767) * 4.096)-(.01/voltage)) / .025' to get rid of negative number problem
	# previous testing did not account for a wire through the sensors with no current
	if not(voltage > 0):
		return -1;
	value = ((float(value)/32768) * 4.096) / .025
...
def getMQTT(mqttServerIP, mqttID):
	keepAliveInterval = 15
	# The MQTT connection will require an MQTT server IP and a sensor ID to subscribe to
	client = MqttClient.MqttClient(mqttServerIP, mqttID, keepAliveInterval)
...
class Sensor:
	...
	def set_value(self):
		...
		if (self.type == "Light"):
			...
			reading = getMQTT(self.mqttIP, self.mqttSensor)
		self.value = reading
